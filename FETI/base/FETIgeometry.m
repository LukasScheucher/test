                ...
                case 'NTS-LM'
                    for n=1+n_convert*N_s_self(int):N_s_self(int)+n_convert*(N_int-N_s_self(int)) % jump through slave-nodes
                        ...
                        if n_conf==0
                            ...
                            if p.int(3,int)==1 % vertical interface
                                if p.posn_int{int}(2,m_C)<p.posn_int{int}(2,n) % slave-node n must be between master-nodes m_C and m_R
                                    m_R=m_C+1;
                                else
                                    m_R=m_C-1;
                                end
                                ...
                                if p.posn_int{int}(1,m_C)<p.posn_int{int}(1,n) % slave-node n must be between master-nodes m_C and m_R
                                    m_R=m_C+1;
                                else
                                    m_R=m_C-1;
                                end
                        ...
                        if interface_boundaries==1
                            ...
                            for l=1:2
                                ...
                                for k=1:2
                                    ...
                                    disp(['Bs(slave)-Eintrag: ' num2str((shapefunc(l,xi_a(1))*shapefunc(k,xi_a(1))+shapefunc(l,xi_b(1))*shapefunc(k,xi_b(1)))*J_seg)])
                                    disp(['Bs(master)-Eintrag: ' num2str(-(shapefunc(l,xi_a(1))*shapefunc(k,xi_a(2))+shapefunc(l,xi_b(1))*shapefunc(k,xi_b(2)))*J_seg)])
                                    ...
                                    p.Bs{subs1}(lag_mult(lm_i+(l-1)*2),p.posn_int{int}(3,n_slave)*2-1)=p.Bs{subs1}(lag_mult(lm_i+(l-1)*2),p.posn_int{int}(3,n_slave)*2-1)+(shapefunc(l,xi_a(1))*shapefunc(k,xi_a(1))+shapefunc(l,xi_b(1))*shapefunc(k,xi_b(1)))*J_seg;
                                    p.Bs{subs2}(lag_mult(lm_i+(l-1)*2),p.posn_int{int}(3,n_master)*2-1)=p.Bs{subs2}(lag_mult(lm_i+(l-1)*2),p.posn_int{int}(3,n_master)*2-1)-(shapefunc(l,xi_a(1))*shapefunc(k,xi_a(2))+shapefunc(l,xi_b(1))*shapefunc(k,xi_b(2)))*J_seg;
